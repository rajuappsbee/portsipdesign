...
$(document).ready(function(){
  ...
  // Tryit JsSIP data.
  var tryit_sip_domain = "www.portsip.com";
  var tryit_ws_uri = "ws://192.168.1.62:10080";
  var invitation_link_pre = "http://www.portsip.com?invited-by="
...
var u          = getQueryString('u'),
	p          = getQueryString('p'),
	to         = $.trim(getQueryString('to')),
	sip_domain = (getQueryString('sip_domain') != null && getQueryString('sip_domain') != 'undefined') ? getQueryString('sip_domain') : null,
	ws_servers = (getQueryString('ws_servers') != null && getQueryString('ws_servers') != 'undefined') ? getQueryString('ws_servers') : null,
	sipStatus  = false;

  //Ìí¼ÓToºÅÂë
  if (/^(\d+)$/.test(to)) $('.destination').val(to);

  if (u && p && sip_domain && ws_servers)
  {
	    sipStatus          = true;
		var display_name   = u,
		password           = p,
		authorization_user = display_name;

		login_display_name.val(display_name);
		login_sip_uri.val(sip_domain);
		login_sip_password.val(password);
		login_ws_servers.val(ws_servers);
  ...
  if (window.CustomJsSIPSettings) {
    ...
    //joe added if uri is null, don't submit it.
    //login_form.submit();
    //if(login_sip_uri.length==0 || login_sip_password.length==0 || login_ws_servers.length==0)
  }
  ...
  function getBalloonContent(item) {
    ...
    case "login_advanced_settings":
        balloon_text = "<p>Set advanced settings</p>";
      break;
  ...
  function phoneInit() {
    ...
    // Otherwise load data from the forms.
      ...
      if (login_sip_uri.val() != "") {
        sip_uri = 'sip:' + display_name + '@' + login_sip_uri.val();
        //sip_uri = $('#sip_uri').val();
      }
      ...
      if (! sip_uri) {
        ...
        Y_U_NO("Please enter the SIP user name and password");
        return false;
